...
import type { AgentResponse as SystemPromptResponse, AgentSession as SystemPromptSession } from '@api/types/agent';
...
import {
  ...
  EmptySessions,
  ActiveSessionsNote,
} from './SystemPromptDetail.styles';
...
const getSessionColumns = (agentId: string): TableColumn<SystemPromptSession>[] => [
  {
    key: 'id',
    header: 'Session ID',
    render: (session) => (
...
export const SystemPromptDetail: FC = () => {
  ...
  const reportLink = agentLink(agentId, `/system-prompt/${systemPrompt.id}/report`);
  ...
  return (
    <PageContainer>
      ...
      <SystemPromptHeader>
        <SystemPromptHeaderLeft>
          <SystemPromptTitle>{systemPrompt.id}</SystemPromptTitle>
          <SystemPromptMeta>
            <span>First seen: <TimeAgo timestamp={systemPrompt.first_seen} /></span>
            <span>Last seen: <TimeAgo timestamp={systemPrompt.last_seen} /></span>
          </SystemPromptMeta>
        </SystemPromptHeaderLeft>
      ...
      <StatsBar>
        <StatItem>
          <StatValue>{systemPrompt.total_sessions}</StatValue>
          <StatLabel>sessions</StatLabel>
        </StatItem>
        <StatDivider />
        <StatItem>
          <StatValue>{formatCompactNumber(systemPrompt.total_messages)}</StatValue>
          <StatLabel>messages</StatLabel>
        </StatItem>
        <StatDivider />
        <StatItem>
          <StatValue>{formatCompactNumber(systemPrompt.total_tokens)}</StatValue>
          <StatLabel>tokens</StatLabel>
        </StatItem>
        <StatDivider />
        <StatItem>
          <StatValue>{systemPrompt.total_tools}</StatValue>
