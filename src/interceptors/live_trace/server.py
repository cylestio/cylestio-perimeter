...
def create_trace_server(insights: InsightsEngine, refresh_interval: int = 2) -> FastAPI:
    ...
    async def api_get_workflow_state(workflow_id: str):
        """Get the current lifecycle state of a workflow."""
        try:
            # Check what exists
            static_sessions = insights.store.get_analysis_sessions(workflow_id=workflow_id)
            dynamic_agents = insights.store.get_all_agents(workflow_id=workflow_id)
            findings = insights.store.get_findings(workflow_id=workflow_id)
            ...
            if not has_static and not has_dynamic:
                state = "NO_DATA"
                recommendation = "Run static analysis from your IDE or dynamic testing through the proxy."
            elif has_static and not has_dynamic:
                state = "STATIC_ONLY"
                recommendation = f"You have {len(open_findings)} open findings. Run dynamic testing to validate."
            elif has_dynamic and not has_static:
                state = "DYNAMIC_ONLY"
                recommendation = "Dynamic data captured. Run static analysis from your IDE for correlation."
            else:
                state = "COMPLETE"
                recommendation = None
    ...
    @app.get("/api/workflow/{workflow_id}/tool-usage")
    async def api_get_workflow_tool_usage(workflow_id: str):
        ...
        try:
            ...
            if not agents:
                return JSONResponse({
                    "workflow_id": workflow_id,
                    "message": "No dynamic sessions found.",
                    "tool_usage": {},
                    "total_sessions": 0,
                })
            ...
            logger.error(f"Error getting workflow tool usage: {e}")
            return JSONResponse({"error": str(e)}, status_code=500)

    @app.get("/api/workflow/{workflow_id}/correlation")
    async def api_get_workflow_correlation(workflow_id: str):
        """Get correlation between static findings and dynamic observations."""
        try:
            # Get static data
            findings = insights.store.get_findings(workflow_id=workflow_id)
            static_sessions = insights.store.get_analysis_sessions(workflow_id=workflow_id)
            ...
            tool_call_counts = {}
            total_dynamic_sessions = 0

            for agent in agents:
                total_dynamic_sessions += agent.total_sessions
                dynamic_tools_used.update(agent.used_tools)
                for tool_name, count in agent.tool_usage_details.items():
                    tool_call_counts[tool_name] = tool_call_counts.get(tool_name, 0) + count

            # Simple correlation: check if findings mention tools that were exercised
            correlations = []
            for finding in findings:
                correlations.append({
                    ...
                    "dynamically_tested": total_dynamic_sessions > 0,
                })

            # Summary
            validated = len([c for c in correlations if c["dynamically_tested"]])
            recommendations = []

            if not agents:
                recommendations.append("No dynamic data. Run your agent through the proxy to validate findings.")
            if not findings:
                recommendations.append("No static findings. Run static analysis from your IDE.")
            if agents and findings:
                recommendations.append("Both static and dynamic data available. Review correlations above.")

            return JSONResponse({
                "workflow_id": workflow_id,
                "static_findings_count": len(findings),
                "dynamic_sessions_count": total_dynamic_sessions,
                "dynamic_tools_used": list(dynamic_tools_used),
                "correlations": correlations,
                "recommendations": recommendations,
            })
        except Exception as e:
            logger.error(f"Error getting workflow correlation: {e}")
            return JSONResponse({"error": str(e)}, status_code=500)

    @app.patch("/api/agent/{agent_id}/info")
    async def api_update_agent_info(agent_id: str, request: Request):
        ...
        try:
            ...
            if not display_name and not description and not workflow_id:
                return JSONResponse(
                    {"error": "At least one of display_name, description, or workflow_id required"},
                    status_code=400
                )

            result = insights.store.update_agent_info(
                agent_id=agent_id,
            ...
            logger.error(f"Error updating agent info: {e}")
            return JSONResponse({"error": str(e)}, status_code=500)
